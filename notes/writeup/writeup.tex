\documentclass[11pt,onecolumn]{IEEEtran}

\renewcommand{\thefootnote}{$\star$}

\usepackage[arrowdel]{physics}
\usepackage{siunitx}
\usepackage{tabularx}
\usepackage{graphicx}
\usepackage{calc}
\usepackage{listings}
\usepackage[margin=0.75in]{geometry}
\usepackage{hyperref}
\usepackage{cleveref}
\usepackage{url}
\usepackage{tikz}
\usepackage{siunitx}

\author{Henry Mitchell}
\title{LoRa Setup Writeup}
\date{}

\lstset{backgroundcolor = \color{lightgray},
  basicstyle = \ttfamily \small,
  breaklines = true,
  breakatwhitespace = true,
  commentstyle = \ttfamily \color{white},
  frame = none,
  numbers = left,
  showstringspaces = false,
  upquote = false}

\begin{document}
\maketitle
\section{Initial Testing}
\subsection{Introduction}
\label{sec:intro}
\input{initial_testing/intro}

\subsection{Materials}
\label{sec:materials}
\input{initial_testing/materials}

\subsection{Connecting Sensors}
\label{sec:setup}
\input{initial_testing/connecting_sensors}

\subsection{Procedure}
\label{sec:procedure}
\input{initial_testing/procedure}

\section{Gateway Communication}
\label{sec:gateway}
\subsection{Introduction}
\label{sec:gateway:intro}
\input{gateway/intro}

\subsection{Materials}
\label{sec:gateway:materials}
\input{gateway/materials}

\subsection{Caveats}
\label{sec:gateway:caveats}
\input{gateway/caveats}

\subsection{Procedure}
\label{sec:gateway:procedure}
\input{gateway/procedure}

\subsection{Results}
\label{sec:gateway:results}
\input{gateway/results}

\subsection{Discussion}
\label{sec:gateway:discussion}
\input{gateway/discussion}

\end{document}

%%% Local Variables:
%%% mode: latex
%%% TeX-master: t
%%% End: